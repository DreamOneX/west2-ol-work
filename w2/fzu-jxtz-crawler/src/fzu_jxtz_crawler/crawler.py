...
from fzu_jxtz_crawler.models import AnnouncementEntry, Attachment
from fzu_jxtz_crawler.parsers import parse_announcement_list, parse_announcement_page
...
def crawl_download_times(
    client: CrawlerClient,
    entries: list[AnnouncementEntry],
    on_download_times_fetched: Callable[[int, int, Attachment], None] | None = None,
    max_workers: int = 5
) -> None:
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_attachment = {
            executor.submit(
                client.get_download_times,
                attachment.file_code,
                attachment.owner_code
            ) : ( idx, attachment )

            for idx, attachment in enumerate([
                attachment for entry in entries
                if entry.attachments is not None
                for attachment in entry.attachments
            ])
        }

        for future in as_completed(future_to_attachment):
            idx, attachment = future_to_attachment[future]
            try:
                download_times = future.result()
                attachment.download_times = download_times
                if on_download_times_fetched:
                    on_download_times_fetched(idx + 1, len(entries), attachment)
            except Exception as e:
                print(f"Error fetching download times for {attachment.file_code}: {e}")
