...
class TestParseEntryFromLi:
    ...
    def test_parse_valid_entry(self, sample_announcement_li: HtmlElement) -> None:
        ...
        assert entry.announcement_id == "001036014352"
    ...
    def test_parse_entry_with_absolute_url(self):
        ...
        <li>
            <span class="doclist_time">2025-11-14</span>
            【教学运行】
            <a href="https://example.com/info/123/456.htm" target="_blank" title="外部链接">外部链接</a>
        </li>
        """
        tree = html.fromstring(html_content)
        li_element = cast(list[HtmlElement], tree.xpath("//li"))[0]
        ...
        assert entry is not None
        assert entry.url == "https://example.com/info/123/456.htm"
        # Since this is a made-up URL, extract_id might behavior weird if not matching pattern
        # but pure check is not part of this specific test unit unless we want to assert ID
