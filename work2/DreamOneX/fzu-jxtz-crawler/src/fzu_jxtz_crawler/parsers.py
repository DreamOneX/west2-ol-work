...
from fzu_jxtz_crawler.url_utils import construct_full_url, derive_content_id_from_url
...
ANNOUNCEMENT_BODY_XPATH = "//*[@class='xl_news']/div"
...
def parse_attachment_from_li(li: HtmlElement) -> Attachment | None:  # pyright: ignore[reportAny]
    ...
    # Create attachment_id from owner_code and file_code
    # This matches the format expected by derive_attachment_id_from_url
    attachment_id = owner.zfill(12) + file_code.zfill(10)
    
    return Attachment(
        attachment_id=attachment_id,
        name=name,
        download_times=0, # Fetch this in next stage
        url=full_url,
        owner_code=owner,
        file_code=file_code,
        local_path=None
...
def parse_entry_from_li(li: HtmlElement) -> AnnouncementEntry | None:  # pyright: ignore[reportAny]
    ...
    # derive ID from url
    announcement_id = derive_content_id_from_url(full_url)
