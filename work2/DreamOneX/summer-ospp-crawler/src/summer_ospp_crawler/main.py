        ...
        with ThreadPoolExecutor(max_workers=args.concurrency) as executor:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    failed_projects.append(p.program_id)
                    logger.warning(f"Failed {count}/{total}: {p.program_name} - {e}")
        
        if failed_projects:
            logger.warning(f"{len(failed_projects)} projects failed to process: {failed_projects}")
    else:
