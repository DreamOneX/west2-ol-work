...
class OSPPCrawler:
    ...
    def fetch_project_list(self, page_size: int = 50, max_pages: int = -1, limit: int = -1) -> list[Project]:
        ...
        while True:
            ...
            try:
                ...
                logger.error(f"HTTP error fetching page {page_num}: {e.response.status_code}")
                break
    ...
    def fetch_project_detail(self, project: Project) -> Project:
        ...
        try:
            ...
            logger.error(f"HTTP error fetching detail for {project.program_id}: {e.response.status_code}")
        except httpx.RequestError as e:
            logger.error(f"Network error fetching detail for {project.program_id}: {e}")
    ...
    def download_pdf(self, project: Project, save_dir: str):
        ...
        try:
            ...
            logger.error(f"HTTP error downloading PDF for {project.program_id}: {e.response.status_code}")
        except httpx.RequestError as e:
            logger.error(f"Network error downloading PDF for {project.program_id}: {e}")
        except OSError as e:
            logger.error(f"File error saving PDF for {project.program_id}: {e}")
